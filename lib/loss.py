...
class AdversarialLoss(nn.Module):
  ...
  def __init__(self, types='nsgan', target_real_label=1.0, target_fake_label=0.0):
    r"""
...
class FacialLoss(nn.Module):
  ...
  def __init__(self):
    super(FacialLoss, self).__init__()
    self.add_module('evolve', Evolve())
    self.criterion = torch.nn.CosineSimilarity()

  def __call__(self, x, y):
    # Compute features
    x_feat, y_feat = self.evolve(x), self.evolve(y)

    # Compute loss
    return torch.mean(self.criterion(x_feat, y_feat))
...
#     model_path = 'data/models/backbone_ir50_ms1m_epoch120.pth'
#     if not os.path.isfile(model_path):
#       model_path = os.path.join('/data/uv_inpainting', model_path)

#     self.backbone = evoLVe_model.IR_50([112, 112])
#     self.backbone.eval()
